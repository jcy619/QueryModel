...
#                                    nn.BatchNorm3d(all_head_dim))
    ...
    def forward(self, x):
        ...
        attn = self.attn_drop(attn) #（bs,num_heads,num_patch,num_patch）self-attn自注意力的query对key的注意力权重
...
class Block(nn.Module):
    ...
    def forward(self, x, return_attention=False):
        y, attn = self.attn(self.norm1(x)) #y(16,65,192) attn(16,12,65,65)
        if return_attention:
            return attn
...
class VisionTransformer(nn.Module):
    ...
    def prepare_tokens(self, x):
        B, nc, w, h = x.shape #B=batch_size*2，是2个global crop
