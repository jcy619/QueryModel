...
def train(args):
    ...
    teacher = utils.MultiCropWrapper(
        ...
        norm_last_layer=args.norm_last_layer, #False
...
def train_one_epoch(student, teacher, teacher_without_ddp, view_pred_loss, data_loader,
    ...
    for it, (images,_) in enumerate(metric_logger.log_every(data_loader, 10, header)):  #_target  #add mask here
        ...
        with torch.cuda.amp.autocast(fp16_scaler is not None):
            ...
            student_output  = student(images) #(80,1024) bs=8,每个图10个view
        ...
        if fp16_scaler is None:
            loss.backward()
            if args.clip_grad:
                param_norms = utils.clip_gradients(student, args.clip_grad) #对student进行梯度剪裁
        ...
        with torch.no_grad():
            ...
            #平滑教师模型的参数，将教师模型的参数与学生模型的参数进行融合。教师模型参数平滑，学生模型的训练起稳定作用。
            names_q, params_q, names_k, params_k = [], [], [], []
            for name_q, param_q in student.module.named_parameters():
                names_q.append(name_q) #学生的参数
                params_q.append(param_q)
            for name_k, param_k in teacher_without_ddp.named_parameters():
                names_k.append(name_k) #教师的参数
                params_k.append(param_k)
            names_common = list(set(names_q) & set(names_k)) #两个集合中的共同的元素，都一样
            params_q = [param_q for name_q, param_q in zip(names_q, params_q) if name_q in names_common]
            params_k = [param_k for name_k, param_k in zip(names_k, params_k) if name_k in names_common]

            for param_q, param_k in zip(params_q, params_k): #教师的参数还要用学生的参数更新一下
    ...
    def __init__(self, out_dim, ncrops, warmup_teacher_temp, teacher_temp,
        ...
        self.teacher_temp_schedule = np.concatenate((
            np.linspace(warmup_teacher_temp,
                        teacher_temp, warmup_teacher_temp_epochs),#等差序列，前10个元素，等差序列0.04~0.07
            np.ones(nepochs - warmup_teacher_temp_epochs) * teacher_temp#后190个元素，全是0.07
        )) #存储每个epoch的teacher temperature

    def forward(self, student_output, teacher_output, epoch):
        """
        Cross-entropy between softmax outputs of the teacher and student networks.
        """
        student_out = student_output / self.student_temp #self.student_temp=0.1 扩大了10倍
        student_out = student_out.chunk(self.ncrops) #ncrops=10 tuple:10--(8,1024)


        # teacher centering and sharpening
        temp = self.teacher_temp_schedule[epoch] #每个epoch的teacher temperature
        teacher_out = F.softmax((teacher_output - self.center) / temp, dim=-1) #(16,1024)
        teacher_out = teacher_out.detach().chunk(2) #tuple：2 -- （8，1024）
        ...
        total_loss /= n_loss_terms #18，2个global和10个student（减去那个和teacher一样的global）

        total_loss = dict( ce_loss=total_loss, loss=total_loss)
        self.update_center(teacher_output) #center不停更新
        return total_loss
...
class DataAugmentation(object):
    def __init__(self, args):
        ...
        self.global_transfo2 = transforms.Compose([
            ...
            utils.GaussianBlur(0.1), #高斯模糊
            utils.Solarization(0.2), #对图像中的像素进行反转
            normalize,
        ])
        # transformation for the local small crops
        self.local_crops_number = args.local_crops_number
        self.local_transfo = transforms.Compose([ #local view ,16*16,(0.2-0.4)
