    ...
    for i, (images, target) in enumerate(train_loader):
        ...
        # Both Cutmix and Mixup
            ...
            if r < args.mix_prob:
                ...
                # Mixup
                else: #把其他某一个的样本与当前样本按一定比例相加
                    images, y_a, y_b, lam = mixup_data(images, target, args) #（8，3，32，32），target（8，）
                    output = model(images)
                    
                    loss = mixup_criterion(criterion, output, y_a, y_b, lam) 
                    
            else:
                output = model(images) #（8，100）
...
def validate(val_loader, model, criterion, lr, args, epoch=None):
    ...
    with torch.no_grad():
        for i, (images, target) in enumerate(val_loader):
            ...
            output = model(images)
            loss = criterion(output, target) #label smoothing cross entrophy
